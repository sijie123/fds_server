...
manager.get('/intervalinsufficient', [
  cookie('authToken').exists().custom(token => authenticateToken(token, 'managers')),
], validate, function (req, res) {
  return db.query("SELECT getdow(day) as day, hour, 5 - num as diff FROM getHourIntervalWorkerCount() as x(day int ,hour int, num int) WHERE num < 5")
    .then(result => res.send(result))
    .catch(err => res.failure(`${err}`))
})
